...
class EigenX():
    ...
    def read_image(self, dataset_path, random=False):
        ...
        img_list = [cv2.imread(os.path.join(dataset_path, i)) for i in img_name_list]
        if self.img_shape is not None:
            self.img_list = [cv2.resize(img, self.img_shape) for img in img_list]
        else:
    ...
    def reconstruct(self, img_path, nums_pc=[10,25,50,75]):
        ...
        vec = (torch.tensor(self.img2vec(img)).cuda() - self.avg_vec).unsqueeze(0)  # [1, h*w]
        ...
        for i, k in enumerate(nums_pc):
            ...
            img_recon = (vec_decode + self.avg_vec).cpu().numpy().reshape(self.img_shape)
...
if __name__ == "__main__":
    ...
    """ Read images """
    dataset_path = "./data/fullbody_anime_girls/ganime_fullbody_ultraclean_256/resized_ganime_fullbody_ultraclean-20220708T155251Z-002/resized_ganime_fullbody_ultraclean"
    ...
    for i in range(1,4):
        ...
        x.reconstruct(img_path, nums_pc=[10,25,50,75,100,200,300,400,500,600])